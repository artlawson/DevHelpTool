...
description: Generate a context-grounded spec file for a new feature or bugfix
category: validation
...
2. If found, extract:
   - **Project vision and identity** — who is this for, what does "done" look like?
   - **Current phase and acceptance criteria** — what milestone are we in?
   - **Decision log** — what has been decided that constrains this spec?
   - **Constraints and non-goals** — what is out of bounds for this project?
   - **Active priorities** — what is the project focused on right now?

3. Hold this context in working memory throughout the entire spec creation process. Reference it when:
...
4. If no context documents are found:
   - Note this in the spec preamble
   - Proceed with standard spec creation
   - Recommend the user create context documents for future work
...
- **Vision fit**: Does this feature serve the project's stated vision and target users?
- **Phase fit**: Does this belong in the current phase, or is it future work?
- **Decision consistency**: Does this conflict with any logged decisions?
- **Non-goal violation**: Does any part of this feature overlap with stated non-goals?

**CRITICAL: Only proceed if the core problem is clearly defined, validated, AND aligned with project context. If uncertain, request additional context.**
...
5. **How this aligns with the project context** (phase, vision, constraints)
...
5. **Context Alignment**: How this feature relates to the project's current phase, vision, and priorities. Reference specific sections of the context documents. Flag any tensions or trade-offs.
6. **Background/Problem Statement**: Why this feature is needed or what problem it solves
7. **Goals**: What we aim to achieve (bullet points)
8. **Non-Goals**: What is explicitly out of scope (bullet points) — cross-reference with project-level non-goals
...
11. **User Experience**: How users will interact with this feature
12. **Testing Strategy**:
    - Unit tests
    - Integration tests
    - E2E tests (if needed)
...
13. **Performance Considerations**: Impact on performance and mitigation strategies
14. **Security Considerations**: Security implications and safeguards
15. **Documentation**: What documentation needs to be created/updated
16. **Implementation Phases**:
    - Phase 1: MVP/Core functionality
    - Phase 2: Enhanced features (if applicable)
    - Phase 3: Polish and optimization (if applicable)
17. **Open Questions**: Any unresolved questions or decisions
18. **References**:
    - Links to related issues, PRs, or documentation
    - External library documentation links
    - Relevant design patterns or architectural decisions
    - **Context documents referenced** (list which sections of PROJECT_CONTEXT, SOUL_DOCUMENT, or PROJECT_OWNER informed this spec)
...
- **Context Alignment**: Verify feature fits current phase and project direction
...
1. **Completeness Check**: All 18 sections meaningfully filled
2. **Consistency Check**: No contradictions between sections
3. **Implementability Check**: Someone could build this from the spec
4. **Context Alignment Check**: Spec is consistent with project context documents
5. **Quality Score**: Rate spec 1-10, only accept 8+
